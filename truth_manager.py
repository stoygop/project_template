...
def _stale_draft_archive_dir(cfg: TruthConfig) -> Path:
    return get_draft_dir(cfg) / "_stale"


def archive_stale_drafts(project: str, expected_ver: int, cfg: TruthConfig) -> List[Path]:
    """Archive any draft(s) whose version does not match expected_ver.

    Strict behavior: if a stale draft exists, it MUST be MOVED into
    <draft_root>/_stale/ with a timestamp suffix. If any move fails, this
    function raises and MUST leave the stale draft(s) in place.

    Returns a list of archive destination paths (may be empty).
    """
    ...
    archive_dir = _stale_draft_archive_dir(cfg)
    archive_dir.mkdir(parents=True, exist_ok=True)
    archived: List[Path] = []
    stamp = _now_stamp()
    for src in sorted(stale):
        # Preserve original filename + add timestamp suffix
        dst = archive_dir / f"{src.stem}_{stamp}{src.suffix}"
        try:
            src.replace(dst)
        except Exception as e:
            raise RuntimeError(f"stale draft archive MOVE failed: src={src} dst={dst} err={e!r}")
        archived.append(dst)
    return archived
...
def ensure_post_artifacts_for_current_truth() -> Tuple[Path, Path, Path]:
    """Ensure required POST artifacts exist for the CURRENT TRUTH_VERSION.

    Guarantees:
      - FULL truth zip exists
      - SLIM truth zip exists
      - last_before_confirm_backup.json exists and points to a readable, valid backup zip

    Returns: (full_zip, slim_zip, backup_zip)
    """
    cfg = TruthConfig.load(CONFIG_JSON)
    project, cur = read_project_and_truth_version()

    # Ensure truth zips exist (create if missing)
    zip_root = REPO_ROOT / cfg.zip_root
    zip_root.mkdir(parents=True, exist_ok=True)
    full_zip = zip_root / f"{project}_TRUTH_V{cur}_FULL.zip"
    slim_zip = zip_root / f"{project}_TRUTH_V{cur}_SLIM.zip"

    if not full_zip.exists():
        make_zip(full_zip, cfg, slim=False, project=project)
    if not slim_zip.exists():
        make_zip(slim_zip, cfg, slim=True, project=project)
    ...
    # Ensure before_confirm marker exists and points to a valid backup zip
    marker = zip_root / "last_before_confirm_backup.json"
    ...
    if marker.exists():
        try:
            ...
            bz = Path(str(payload.get("backup_zip", ""))).expanduser()
            if bz.exists():
                validate_repo_backup_zip(bz)
                backup_zip = bz
        except Exception:
            backup_zip = None
    ...
    return full_zip, slim_zip, backup_zip
...
def main(argv: List[str] | None = None) -> int:
    ...
    ap_archive = sub.add_parser(
        ...
        help="archive any stale draft(s) that do not match the expected next TRUTH version",
    )
    ap_archive.add_argument("--expected", required=True, type=int, help="expected draft version (NEXT VERSION SLOT)")

    sub.add_parser(
        "ensure-artifacts",
        help="ensure required POST artifacts exist for the CURRENT truth (FULL/SLIM zips + before_confirm marker)",
    )
    ...
    if args.cmd == "archive-stale-drafts":
        cfg = TruthConfig.load(CONFIG_JSON)
        expected = int(args.expected)
        archived = archive_stale_drafts(project, expected_ver=expected, cfg=cfg)
        for p in archived:
            print(f"OK: archived stale draft -> {p}")
        if not archived:
            print("OK: no stale drafts")
        return 0

    if args.cmd == "ensure-artifacts":
        full_zip, slim_zip, backup_zip = ensure_post_artifacts_for_current_truth()
        print(f"OK: ensured FULL: {full_zip}")
        print(f"OK: ensured SLIM: {slim_zip}")
        print(f"OK: ensured before_confirm backup: {backup_zip}")
        return 0
