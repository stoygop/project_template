...
def confirm_draft() -> Tuple[int, Path, Path]:
    ...
    if draft_ver != new_ver:
        raise RuntimeError(
            f"pending draft mismatch: found TRUTH_V{draft_ver} draft at {draft_path} "
            f"but expected TRUTH_V{new_ver} (based on app/version.py). "
            f"This usually means a stale draft file is blocking minting; delete the draft file and retry."
        )
...
def main(argv: List[str] | None = None) -> int:
    ...
    ap_ensure = sub.add_parser("ensure-artifacts", help="ensure FULL+SLIM truth zips exist for a given version; optionally create/update before_confirm backup marker")
    ap_ensure.add_argument("--ver", default="latest", help="version number (e.g. 11) or 'latest' (default: latest)")
    ap_ensure.add_argument("--backup", action="store_true", help="also create a before_confirm repo backup zip and update last_before_confirm_backup.json marker")
    ap_ensure.add_argument("--json", action="store_true", help="emit machine-readable json")
    ...
    if args.cmd == "ensure-artifacts":
        ...
        if ver_arg == "latest":
            ...
            try:
                ...
                raise RuntimeError("--ver must be an integer or 'latest'")

        zip_root = REPO_ROOT / cfg.zip_root
        zip_root.mkdir(parents=True, exist_ok=True)

        full_zip = zip_root / f"{project}_TRUTH_V{target_ver}_FULL.zip"
        slim_zip = zip_root / f"{project}_TRUTH_V{target_ver}_SLIM.zip"
        ...
        if getattr(args, "json", False):
            out = {
                ...
                "slim_zip": str(slim_zip),
                "backup_zip": str(backup_zip) if backup_zip else None,
            }
            print(json.dumps(out, indent=2))
        else:
            print(f"OK: Truth artifacts ensured (FULL + SLIM) for TRUTH_V{target_ver}")
            print(f"FULL: {full_zip}")
            print(f"SLIM: {slim_zip}")
            if backup_zip:
                print(f"OK: before_confirm backup validated: {backup_zip}")
        return 0
