...
if ($latestType -ne "CONFIRM") {
  ...
  if ($respType.ToLower() -eq "y") {
    ...
    # V12: confirming the latest truth must also ensure required POST artifacts exist
    & python -m tools.truth_manager ensure-artifacts --ver latest --backup
    if ($LASTEXITCODE -ne 0) { Fail "truth_manager ensure-artifacts failed" }

    & python -m tools.doctor --phase post
    if ($LASTEXITCODE -ne 0) { Fail "doctor post failed after confirm" }
