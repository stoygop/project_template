...
# V13: stale drafts must be automatically archived (MOVED) before proceeding.
if ($draft) {
  $dv0 = [int]$draft.ver
  if ($dv0 -ne $next) {
    Write-Host "" 
    Write-Host ("DIAG: stale draft detected: TRUTH_V{0} but expected TRUTH_V{1} (NEXT VERSION SLOT)" -f $dv0, $next) -ForegroundColor Yellow
    Write-Host ("DIAG: draft path: {0}" -f $draft.path) -ForegroundColor Yellow
    Write-Host "STALE DRAFT detected — archiving is required before proceeding." -ForegroundColor Yellow
    & python -m tools.truth_manager archive-stale-drafts --expected $next
    if ($LASTEXITCODE -ne 0) { Fail "truth_manager archive-stale-drafts failed" }
    # refresh status after archival
    $statusJson = (& python -m tools.truth_manager status --json)
    if ($LASTEXITCODE -ne 0 -or -not $statusJson) { Fail "could not determine truth status from tools.truth_manager" }
    try { $st = $statusJson | ConvertFrom-Json } catch { Fail "could not parse truth status json" }
    $cur = [int]$st.confirmed
    $next = [int]$st.next
    $draft = $st.draft_pending
    $latestType = $st.latest_header_type
...
if ($latestType -ne "CONFIRM") {
  ...
  if ($respType.ToLower() -eq "y") {
    ...
    # V13: confirm-without-version-bump must ensure artifacts exist + run doctor(post)
    & python -m tools.truth_manager ensure-artifacts
    if ($LASTEXITCODE -ne 0) { Fail "truth_manager ensure-artifacts failed" }
    & python -m tools.doctor --phase post
    if ($LASTEXITCODE -ne 0) { Fail "doctor(post) failed" }
    exit 0
  }
}
